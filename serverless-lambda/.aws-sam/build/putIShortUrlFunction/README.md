...
```bash
my-application$ sam local invoke putIShortUrlFunction --event events/event-post-item.json
...
```bash
my-application$ sam logs -n putIShortUrlFunction --stack-name sam-app --tail
```
...
```bash
sam delete --stack-name serverless-lambda
```
